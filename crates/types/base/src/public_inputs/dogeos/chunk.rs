...
impl From<(scroll::chunk::ChunkInfo, DogeOsChunkInfoExtras)> for DogeOsChunkInfo {
    fn from(
        value: (scroll::chunk::ChunkInfo, DogeOsChunkInfoExtras),
    ) -> Self {
        DogeOsChunkInfo {
            inner: value.0,
            extras: value.1,
...
impl MultiVersionPublicInputs for DogeOsChunkInfo {
    fn pi_by_version(&self, version: Version) -> Vec<u8> {
        ...
        scroll_chunk_pi.extend_from_slice(&self.extras.start_blockhash);
        scroll_chunk_pi.extend_from_slice(&self.extras.end_blockhash);
        scroll_chunk_pi
    }
