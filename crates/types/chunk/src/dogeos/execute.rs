...
use crate::dogeos::DogeOsChunkWitnessExtras;
use crate::dogeos::types::{MOAT_CONTRACT_ADDRESS, handleL1MessageCall};
use crate::scroll::relayMessageCall;
use alloy_primitives::U256;
use alloy_sol_types::SolCall;
use bridge_adapters_zk::ZkVerifierExt;
...
pub fn execute(witness: DogeOsChunkWitness) -> Result<DogeOsChunkInfo, String> {
    ...
    verify_deposits(&witness.extras, &l1_messages)?;
    ...
    let info_extras = DogeOsChunkInfoExtras {
        start_blockhash: start_blockhash.into(),
        end_blockhash: end_blockhash.into(),
    };
    println!("info_extras = {:?}", info_extras);
...
fn verify_deposits(
    extras: &DogeOsChunkWitnessExtras,
    l1_messages: &[TxL1Message],
) -> Result<(), String> {
    HeaderVerifier
        .verify_envelope(&extras.header, &extras.verifier_context)
        .map_err(|e| format!("dogeos deposit header verification failed: {e}"))?;
    assert_eq!(
        &extras.header.statement,
        &extras.midstate.statement.header_range
    );
    MidstateVerifier
        .verify_envelope(&extras.midstate, &extras.verifier_context)
        .map_err(|e| format!("dogeos deposit midstate verification failed: {e}"))?;

    for (deposit, l1_message) in extras
        .midstate
        .statement
        .expected_deposits
        .iter()
        .zip_eq(l1_messages)
    {
        ...
        assert_eq!(
            relay_call.sender,
            deposit.txid[..20],
            "dogeos synthetic relay call sender mismatch"
        );
        ...
        assert_eq!(
            relay_call.target, MOAT_CONTRACT_ADDRESS,
            "dogeos relay call target mismatch"
        );
        ...
        let amount = U256::from(bridge_transforms::convert_doge_to_eth_units(
            deposit.amount_sats,
        ));
        ...
        assert_eq!(
            relay_call.value, amount,
            "dogeos relay call amount mismatch"
        );
        // nonce/queueIndex check is skipped, as it's guaranteed by rolling hash

        let moat_call: handleL1MessageCall =
            handleL1MessageCall::abi_decode(relay_call.message.as_ref())
        ...
        assert_eq!(
            moat_call.target, deposit.evm_recipient,
            "dogeos deposit recipient mismatch"
        );
