...
fn write_commitment(output: &str, commitment: [u32; 8]) -> eyre::Result<()> {
    let content = format!(
        "#![cfg_attr(rustfmt, rustfmt_skip)]\n//! Generated by crates/build-guest. DO NOT OVERWRITE!\n\npub const COMMIT: [u32; 8] = {:?};\n",
        commitment
    );
...
pub fn main() -> eyre::Result<()> {
    ...
    for project_name in &project_names {
        ...
        write_commitment(
            &format!("{dir_project}/{project_name}_leaf_commit.rs"),
            leaf_vm_verifier_commit,
        )?;
        if project_name == &"bundle" {
            let digest_2 = compress_commitment(&leaf_vm_verifier_commit)
                .to_bytes()
                .into_iter()
                .rev()
                .collect::<Vec<u8>>();
            let filename = format!("{dir_workspace}/crates/circuits/bundle-circuit/digest_2",);
    ...
    for project_name in &project_names {
        ...
        for build_config in build_configs {
            ...
            write_commitment(
                &format!(
                    "{dir_project}/{project_name}_exe{}_commit.rs",
            ...
            if project_name == &"bundle" {
                let digest_1 = compress_commitment(&exe_commit)
                    .to_bytes()
                    .into_iter()
                    .rev()
                    .collect::<Vec<u8>>();
