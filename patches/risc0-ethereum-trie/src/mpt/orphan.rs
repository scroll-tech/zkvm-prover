...
impl Trie {
    ...
    {
        self.0
            .resolve_orphan(NibbleSlice::from(&Nibbles::unpack(key)), proof)
    }
}
...
impl<M: Memoization + Clone> Node<M> {
    ...
    fn diverging<'a>(&'a self, key: NibbleSlice<'a>) -> Option<(&'a Node<M>, NibbleSlice<'a>)> {
        match self {
            ...
            Node::Extension(prefix, child, _) => key
                .strip_prefix(prefix)
                .map_or(Some((self, key)), |tail| child.diverging(tail)),
...
mod tests {
    ...
    fn leaf_orphan() {
        ...
        assert!(
            panic::catch_unwind(|| trie.clone().remove(key)).is_err(),
            "Removal should panic"
        );
